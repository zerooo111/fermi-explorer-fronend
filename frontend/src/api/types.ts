...
/**
 * Convert microsecond timestamp to ConvertedTimestamp using bn.js for safety
 */
export const convertTimestamp = (
  microseconds: number | string,
): ConvertedTimestamp => {
  // Use the new big number utilities for safe conversion
  const { microsecondsToMilliseconds } = require('@/lib/bigNumbers')
  const milliseconds = microsecondsToMilliseconds(microseconds)
  const date = new Date(milliseconds)
  
  return {
    microseconds:
      typeof microseconds === 'string'
        ? parseInt(microseconds, 10)
        : microseconds,
    milliseconds,
    date,
...
/**
 * Calculate age in seconds from timestamp using bn.js for safety
 */
export const calculateAge = (timestamp: number | string): number => {
  // Use the new big number utilities for safe calculation
  const { calculateAgeFromMicroseconds } = require('@/lib/bigNumbers')
 ...
 */
export const enhanceTransactionData = (
  transaction: TransactionData,
